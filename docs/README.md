...
* 使用`org.springframework.web.context.ContextLoaderListener`来加载spring配置启动
* 使用`org.springframework.web.servlet.DispatcherServlet`来加载spring配置启动(springMVC)

#### 安装使用zookeeper

下载安装原生[zookeeper](http://zookeeper.apache.org)即可

#### 下载依赖

下载安装[zkspring.jar](../bin/zkspring-0.0.1-SNAPSHOT.jar)依赖
```java
<dependency>
