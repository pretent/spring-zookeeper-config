...
@Setter
@Getter
public class ZkPropertyPlaceholderConfigurer extends
	...
	/**
	 * 从ApplicationContextAware接口实现实现
	 * 
	 * 为了拿到Context对象
	 * 
	...
	public void setApplicationContext(ApplicationContext applicationContext)
		...
		// 保存需要监听的properties文件
		ApplicationContextUtils.setProperties(properties);
		// 保存需要监听的xml文件
		ApplicationContextUtils.setXmls(xmls);
	}
	...
	protected void processProperties(
		...
		String[] configs = (String[]) ArrayUtils.addAll(properties, xmls);
		new ZkWacher(this.servers, configs).listen();
		this.zkClient.close();
		// new Curator().listener();
	}
	...
	private void fillProperties(Properties props, byte[] data)
		...
		// bre = new BufferedReader(new InputStreamReader(new
		// FileInputStream(new File("/Users/user/app.properties"))));
		try {
